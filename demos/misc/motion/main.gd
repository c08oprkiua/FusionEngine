# based on godot's motion sample
extends Node
...
func _process(delta):
	...
	get_node("Control/Accelerometer").set_text("Accelerometer: " + str(acc) + ", gravity: " + str(grav))
	get_node("Control/Gyroscope").set_text("Gyroscope: " + str(gyro))
	...
	get_node("acc").set_transform(get_basis_for_arrow(grav))
	...
	#var mag_and_grav = get_node("grav")
	...
	var gyro_and_grav = get_node("grav")
	var new_basis = rotate_by_gyro(gyro, gyro_and_grav.get_transform(), delta).orthonormalized()
	gyro_and_grav.set_transform(drift_correction(new_basis, grav))
...
func rotate_by_gyro(p_gyro, p_basis, p_delta):
	...
	rotate = rotate.rotated(p_basis.basis.z, -p_gyro.z * p_delta)
...
func drift_correction(p_basis, p_grav):
	...
	var dot = p_basis.basis.y.dot(real_up)
