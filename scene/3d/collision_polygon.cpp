...
void CollisionPolygon3D::_add_to_collision_object(Object *p_obj) {


	CollisionObject3D *co = p_obj->cast_to<CollisionObject3D>();
	ERR_FAIL_COND(!co);
...
#if 0
		Ref<ConcavePolygonShape3D> concave = memnew( ConcavePolygonShape3D );
...
void CollisionPolygon3D::_update_parent() {

	Node *parent = get_parent();
	if (!parent)
		return;
	CollisionObject3D *co = parent->cast_to<CollisionObject3D>();
	if (!co)
		return;
...
void CollisionPolygon3D::set_build_mode(BuildMode p_mode) {
...
CollisionPolygon3D::BuildMode CollisionPolygon3D::get_build_mode() const{
...
void CollisionPolygon3D::_bind_methods() {
	...
	ObjectTypeDB::bind_method(_MD("_add_to_collision_object"),&CollisionPolygon3D::_add_to_collision_object);
	ObjectTypeDB::bind_method(_MD("set_polygon","polygon"),&CollisionPolygon3D::set_polygon);
	ObjectTypeDB::bind_method(_MD("get_polygon"),&CollisionPolygon3D::get_polygon);

	ObjectTypeDB::bind_method(_MD("set_depth","depth"),&CollisionPolygon3D::set_depth);
	ObjectTypeDB::bind_method(_MD("get_depth"),&CollisionPolygon3D::get_depth);

	ObjectTypeDB::bind_method(_MD("set_build_mode"),&CollisionPolygon3D::set_build_mode);
	ObjectTypeDB::bind_method(_MD("get_build_mode"),&CollisionPolygon3D::get_build_mode);
